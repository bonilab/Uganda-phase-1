...
void Model::set_treatment_strategy(const int &strategy_id) {
  treatment_strategy_ =
      strategy_id == -1 ? nullptr : config_->strategy_db()[strategy_id];
...
void Model::set_treatment_coverage(ITreatmentCoverageModel* tcm) {
  if (treatment_coverage_ != tcm) {
    if (tcm->p_treatment_less_than_5.empty()
        || tcm->p_treatment_more_than_5.empty()) {
      // copy current value
      tcm->p_treatment_less_than_5 =
          treatment_coverage_->p_treatment_less_than_5;
      tcm->p_treatment_more_than_5 =
          treatment_coverage_->p_treatment_more_than_5;
    }
...
void Model::initialize(int job_number, const std::string &path) {
  ...
  if (!verify_configuration()) {
    std::cerr << "ERROR! Unable to continue execution with current "
                 "configuration, see error logs.\n";
    exit(EXIT_FAILURE);
  }
  ...
  try {
    ...
    LOG(ERROR) << "Runtime error encountered while initializing reporter: "
               << ex.what();
    exit(EXIT_FAILURE);
  }
  ...
  scheduler_->set_days_between_notifications(
      config_->days_between_notifications());
  ...
  LOG(INFO) << fmt::format("Location count: {0}",
...
void Model::yearly_update() { data_collector_->yearly_update(); }
...
bool Model::verify_configuration() {
  ...
  if (config_->death_rate_by_age_class().size()
      != config_->number_of_age_classes()) {
    LOG(ERROR) << fmt::format(
        "Number of death rates ({}) does not match the "
        "number of age classes ({})!",
        config_->death_rate_by_age_class().size(),
        config_->number_of_age_classes());
    valid = false;
  }
  ...
  if (config_->birth_rate() > 0.05) {
    LOG(WARNING) << fmt::format(
        "Unusually high birth rate of {} ({}/1000 individuals)",
  ...
  if (config_->birth_rate() >= 1) {
    LOG(ERROR) << fmt::format(
        "Birth rate should not exceed 1.0, did you mean "
        "{} ({}/1000 individuals)?",
