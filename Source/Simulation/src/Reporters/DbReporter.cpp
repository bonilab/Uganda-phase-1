...
void DbReporter::initialize(int job_number, const std::string &path) {
  ...
  LOG(INFO) << fmt::format("Running configuration {}, replicate {}.", config_id,
                           replicate);
}
...
void DbReporter::prepare_configuration(pqxx::connection* connection) {
  ...
  std::string query =
      fmt::format(SELECT_CONFIGURATION, db.quote(yaml), db.quote(filename));
  ...
  if (SpatialData::get_instance().has_raster()) {
    ...
    query =
        fmt::format(INSERT_CONFIGURATION_RASTER, db.quote(yaml), db.quote(yaml),
                    db.quote(filename), header.number_columns,
                    header.number_rows, db.quote(header.x_lower_left_corner),
                    db.quote(header.y_lower_left_corner), header.cellsize);
  } else {
  ...
  if (Model::MODEL->study_number() != -1) {
    ...
    LOG(INFO) << "Associating study number " << study_id
              << " with configuration.";
  ...
  for (auto ndx = 0; ndx < Model::CONFIG->number_of_locations(); ndx++) {
    ...
    if (districts) {
      ...
      query.append(
          fmt::format(INSERT_LOCATION, config_id, ndx, x, y, location.beta));
    }
  }
...
void DbReporter::prepare_replicate(pqxx::connection* connection) {
  ...
  std::string query =
      fmt::format(INSERT_REPLICATE, config_id, Model::RANDOM->seed(), movement,
                  get_genotype_level());
  pqxx::work db(*connection);
  pqxx::result result = db.exec(query);
...
void DbReporter::monthly_report() {
  ...
  while (attempt < RETRY_LIMIT) {
    ...
    if (do_monthly_report()) { return; }
    attempt++;
  }

  // Something went wrong, make sure it is in the logs and end the simulation
  LOG(ERROR) << "Unable to perform monthly report action after " << attempt
             << " attempts!";
...
bool DbReporter::do_monthly_report() {
  ...
  try {
    ...
    std::string query = fmt::format(INSERT_COMMON, replicate, days_elapsed,
                                    model_time, seasonal_factor);
...
void DbReporter::monthly_genome_data(int id, std::string &query) {
  ...
  for (unsigned int location = 0; location < index->vPerson().size();
       location++) {
    ...
    //
    // NOTE since the database was updated to store the weighted occurrences and
    // infected individuals, the weighted frequency is redundant.
    if (infectedIndividuals != 0) {
      ...
      for (unsigned int genotype = 0; genotype < genotypes; genotype++) {
        ...
        query.append(fmt::format(
            INSERT_GENOTYPE_ROW, id, location_index[location], genotype,
    ...
    query.append(fmt::format(UPDATE_INFECTED_INDIVIDUALS, infectedIndividuals,
                             id, location_index[location]));
  }
}
...
void DbReporter::monthly_infected_individuals(int id, std::string &query) {
  ...
  for (unsigned int location = 0; location < index->vPerson().size();
    ...
    // Append the infected count update
    query.append(fmt::format(UPDATE_INFECTED_INDIVIDUALS, infected_individuals,
                             id, location_index[location]));
  }
}
