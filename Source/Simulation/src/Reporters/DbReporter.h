 ...
 * DbReporter.h
 *
 * Define the DbReporter class which is used to insert relevant information from
 * the model into the database during model execution.
 */
#ifndef DBREPORTER_H
#define DBREPORTER_H
...
private:
  const std::string INSERT_COMMON =
      "INSERT INTO sim.MonthlyData (ReplicateId, DaysElapsed, ModelTime, "
      "SeasonalFactor, EntryTime) "
      "VALUES ({}, {}, {}, {}, now()) RETURNING id;";

  const std::string INSERT_CONFIGURATION =
      "INSERT INTO sim.Configuration (Yaml, MD5, FileName) VALUES ({}, "
      "md5({}), {}) RETURNING ID;";

  const std::string INSERT_CONFIGURATION_RASTER =
      "INSERT INTO sim.Configuration (Yaml, MD5, FileName, ncols, nrows, "
  ...
  const std::string INSERT_LOCATION_DISTRICT =
      "INSERT INTO sim.Location (ConfigurationId, Index, X, Y, Beta, District) "
      "VALUES ({}, {}, {}, {}, {}, {});";

  const std::string INSERT_REPLICATE =
      "INSERT INTO replicate (ConfigurationId, Seed, StartTime, Movement, "
      "AggregationLevel) VALUES ({}, {}, now(), "
      "'{}', '{}') RETURNING id;";
  ...
  const std::string SELECT_LOCATION =
      "SELECT id, index FROM sim.location WHERE ConfigurationId = {} ORDER BY "
      "index;";
...
protected:
  const std::string INSERT_GENOTYPE_PREFIX =
      "INSERT INTO sim.MonthlyGenomeData (MonthlyDataId, Location, GenomeId, "
      "Occurrences, ClinicalOccurrences, "
      "Occurrences0to5, Occurrences2to10, WeightedOccurrences) VALUES ";

  // To be appended to INSERT_GENOTYPE_PREFIX, note that the last character must
  // be replaced with a semicolon
  const std::string INSERT_GENOTYPE_ROW = "({}, {}, {}, {}, {}, {}, {}, {}),";

  const std::string INSERT_SITE_PREFIX =
      "INSERT INTO sim.MonthlySiteData (MonthlyDataId, Location, Population, "
      "ClinicalEpisodes, Treatments, EIR, "
      "PfPrUnder5, PfPr2to10, PfPrAll, TreatmentFailures, NonTreatment, "
      "Under5Treatment, Over5Treatment) VALUES ";

  // To be appended to the INSERT_SITE_PREFIX, note that the last character must
  // be replaced with a semicolon
  const std::string INSERT_SITE_ROW =
      "({}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}),";

  const std::string UPDATE_INFECTED_INDIVIDUALS =
      "UPDATE sim.MonthlySiteData SET InfectedIndividuals = {} WHERE "
      "MonthlyDataId = {} AND Location = {};";
