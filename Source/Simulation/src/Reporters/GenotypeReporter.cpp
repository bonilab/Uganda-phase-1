...
// Output genotype frequencies by the weighted number of parasite-positive
// individuals carrying genotype X / total number of parasite-positive
// individuals (the weights for each person describe the fraction of their
// clonal populations carrying genotype X; e.g. an individual host with five
// clonal infections two of which carry genotype X would be given a weight of
// 2/5). \param out The output stream.
    ...
    for (auto hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
      ...
      for (std::size_t ac = 0; ac < index->vPerson()[0][0].size(); ac++) {
        for (auto i = 0ull; i < index->vPerson()[loc][hs][ac].size(); i++) {
          ...
          for (auto* parasite_population :
               *(person->all_clonal_parasite_populations()->parasites())) {
            // Create an entry for the genotype ID, or update the count
            if (individual_genotype_map.find(
                    parasite_population->genotype()->genotype_id())
                == individual_genotype_map.end()) {
              individual_genotype_map[parasite_population->genotype()
                                          ->genotype_id()] = 1;
            } else {
              individual_genotype_map[parasite_population->genotype()
                                          ->genotype_id()] += 1;
...
void GenotypeReporter::output_genotype_weighted_header(std::ofstream &out) {
  ...
  out << "Genotype frequencies by the weighted number of parasite-positive "
         "individuals carrying genotype"
      << Tsv::end_line;

  // Location and genotype
  for (std::size_t location = 0;
       location < Model::CONFIG->number_of_locations(); location++) {
    for (std::size_t type = 0; type < Model::CONFIG->number_of_parasite_types();
         type++) {
