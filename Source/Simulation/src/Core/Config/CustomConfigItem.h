...
public:
  number_of_age_classes(const std::string &name,
                        const unsigned long &default_value, Config* config)
      : ConfigItem<unsigned long>(name, default_value, config) {}
...
public:
  number_of_locations(const std::string &name,
                      const unsigned long &default_value, Config* config)
      : ConfigItem<unsigned long>(name, default_value, config) {}
  // Update the number of locations based upon the location_db size
  void set_value();

  // Update the number of locations based upon the location_db size, the node is
  // ignored
  void set_value(const YAML::Node &node) override;
};

class spatial_distance_matrix
    : public ConfigItem<std::vector<std::vector<double>>> {
public:
  spatial_distance_matrix(const std::string &name,
                          const std::vector<std::vector<double>> &default_value,
                          Config* config)
      : ConfigItem<std::vector<std::vector<double>>>(name, default_value,
                                                     config) {}
...
public:
  ...
  explicit seasonal_info(const std::string &name, ISeasonalInfo* default_value,
...
public:
  // constructor
  explicit spatial_model(const std::string &name,
                         Spatial::SpatialModel* default_value,
                         Config* config = nullptr)
      : IConfigItem(config, name), value_{default_value} {}
  ...
  virtual Spatial::SpatialModel* operator()() { return value_; }
...
public:
  // constructor
  explicit immune_system_information(const std::string &name,
                                     ImmuneSystemInformation default_value,
                                     Config* config = nullptr)
  ...
  virtual ImmuneSystemInformation &operator()() { return value_; }
...
public:
  // constructor
  explicit genotype_db(const std::string &name, GenotypeDatabase* default_value,
                       Config* config = nullptr)
      : IConfigItem(config, name), value_{default_value} {}
  ...
  virtual GenotypeDatabase* operator()() { return value_; }
...
public:
  number_of_parasite_types(const std::string &name,
                           const unsigned long &default_value, Config* config)
      : ConfigItem<unsigned long>(name, default_value, config) {}
...
public:
  // constructor
  explicit drug_db(const std::string &name, DrugDatabase* default_value,
                   Config* config = nullptr)
      : IConfigItem(config, name), value_{default_value} {}
  ...
  virtual DrugDatabase* operator()() { return value_; }
...
public:
  // constructor
  explicit EC50_power_n_table(const std::string &name,
                              DoubleVector2 default_value,
                              Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual DoubleVector2 &operator()() { return value_; }
...
public:
  // constructor
  explicit circulation_info(const std::string &name,
                            RelativeMovingInformation default_value,
                            Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual RelativeMovingInformation &operator()() { return value_; }
...
public:
  // constructor
  explicit relative_bitting_info(const std::string &name,
                                 RelativeBittingInformation default_value,
                                 Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual RelativeBittingInformation &operator()() { return value_; }
...
public:
  // constructor
  explicit therapy_db(const std::string &name, TherapyPtrVector default_value,
                      Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual TherapyPtrVector &operator()() { return value_; }
...
public:
  // constructor
  explicit strategy_db(const std::string &name, StrategyPtrVector default_value,
                       Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual StrategyPtrVector &operator()() { return value_; }
...
public:
  // constructor
  explicit initial_parasite_info(const std::string &name,
                                 std::vector<InitialParasiteInfo> default_value,
                                 Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual std::vector<InitialParasiteInfo> &operator()() { return value_; }
...
public:
  // constructor
  explicit bitting_level_generator(
      const std::string &name, MultinomialDistributionGenerator default_value,
      Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
  ...
  virtual MultinomialDistributionGenerator &operator()() { return value_; }
...
public:
  // constructor
  explicit moving_level_generator(
      const std::string &name, MultinomialDistributionGenerator default_value,
      Config* config = nullptr)
      : IConfigItem(config, name), value_{std::move(default_value)} {}
...
public:
  start_of_comparison_period(const std::string &name, const int &default_value,
                             Config* config)
      : ConfigItem<int>(name, default_value, config) {}
...
public:
  // constructor
  explicit prob_individual_present_at_mda_distribution(
