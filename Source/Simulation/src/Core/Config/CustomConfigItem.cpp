...
void total_time::set_value(const YAML::Node &node) {
  value_ = (date::sys_days{config_->ending_date()}
            - date::sys_days(config_->starting_date()))
               .count();
}
...
void spatial_distance_matrix::set_value(const YAML::Node &node) {
  ...
  for (auto from_location = 0ul; from_location < config_->number_of_locations();
       from_location++) {
    value_[from_location].resize(
        static_cast<unsigned long long int>(config_->number_of_locations()));
    for (auto to_location = 0ul; to_location < config_->number_of_locations();
         to_location++) {
      value_[from_location][to_location] =
...
void immune_system_information::set_value(const YAML::Node &node) {
  ...
  value_.duration_for_fully_immune =
      read_node<double>(is_node, "duration_for_fully_immune");
  value_.duration_for_naive = read_node<double>(is_node, "duration_for_naive");

  const auto mean_initial_condition =
      read_node<double>(is_node, "mean_initial_condition");
  const auto sd_initial_condition =
      read_node<double>(is_node, "sd_initial_condition");

  if (NumberHelpers::is_zero(sd_initial_condition)) {
    value_.alpha_immune = mean_initial_condition;
    value_.beta_immune = 0.0;
  } else {
    value_.alpha_immune = mean_initial_condition * mean_initial_condition
                              * (1 - mean_initial_condition)
                              / (sd_initial_condition * sd_initial_condition)
                          - mean_initial_condition;
    value_.beta_immune =
        value_.alpha_immune / mean_initial_condition - value_.alpha_immune;
  }

  value_.immune_inflation_rate =
      read_node<double>(is_node, "immune_inflation_rate");

  value_.max_clinical_probability =
      read_node<double>(is_node, "max_clinical_probability");

  value_.immune_effect_on_progression_to_clinical =
      read_node<double>(is_node, "immune_effect_on_progression_to_clinical");

  value_.age_mature_immunity =
      read_node<double>(is_node, "age_mature_immunity");
  value_.factor_effect_age_mature_immunity =
      read_node<double>(is_node, "factor_effect_age_mature_immunity");
  ...
  value_.c_min = pow(
      10, -(config_->parasite_density_level().log_parasite_density_asymptomatic
...
void genotype_db::set_value(const YAML::Node &node) {
  ...
  for (auto i = 0; i < number_of_genotypes; i++) {
    auto* int_genotype =
        new Genotype(i, config_->genotype_info(), value_->weight());
...
drug_db::~drug_db() { ObjectHelpers::delete_pointer<DrugDatabase>(value_); }
...
void drug_db::set_value(const YAML::Node &node) {
  ...
  for (unsigned int drug_id = 0; drug_id < node[name_].size(); drug_id++) {
    ...
    dt->set_maximum_parasite_killing_rate(
        dt_node["maximum_parasite_killing_rate"].as<double>());
    dt->set_n(dt_node["n"].as<double>());

    for (std::size_t i = 0;
         i < dt_node["age_specific_drug_concentration_sd"].size(); i++) {
    ...
    if (dt_node["age_specific_drug_absorption"]) {
      for (std::size_t i = 0;
           i < dt_node["age_specific_drug_absorption"].size(); i++) {
        dt->age_specific_drug_absorption()[i] =
            dt_node["age_specific_drug_absorption"][i].as<double>();
      }
    }
    ...
    dt->selecting_alleles().assign(dt_node["affecting_loci"].size(),
                                   IntVector());
    for (std::size_t i = 0; i < dt_node["affecting_loci"].size(); i++) {
      for (std::size_t j = 0; j < dt_node["selecting_alleles"][i].size(); j++) {
        dt->selecting_alleles()[i].push_back(
            dt_node["selecting_alleles"][i][j].as<int>());
      }
    }
...
void EC50_power_n_table::set_value(const YAML::Node &node) {
  ...
  for (unsigned int g_id = 0; g_id < config_->genotype_db()->size(); g_id++) {
    for (unsigned int i = 0; i < config_->drug_db()->size(); i++) {
      value_[g_id].push_back(config_->drug_db()->at(i)->infer_ec50(
          config_->genotype_db()->at(g_id)));
    }
  }
...
void circulation_info::set_value(const YAML::Node &node) {
  ...
  value_.max_relative_moving_value =
      info_node["max_relative_moving_value"].as<double>();

  value_.number_of_moving_levels =
      info_node["number_of_moving_levels"].as<int>();

  value_.scale = info_node["moving_level_distribution"]["Exponential"]["scale"]
                     .as<double>();

  value_.mean =
      info_node["moving_level_distribution"]["Gamma"]["mean"].as<double>();
  value_.sd =
      info_node["moving_level_distribution"]["Gamma"]["sd"].as<double>();
  ...
  const auto max =
      value_.max_relative_moving_value - 1;  // maxRelativeBiting -1
  ...
  assert((unsigned)value_.number_of_moving_levels
         == value_.v_moving_level_density.size());
  assert((unsigned)value_.number_of_moving_levels
         == value_.v_moving_level_value.size());
  assert(fabs(t - 1) < 0.0001);

  value_.circulation_percent = info_node["circulation_percent"].as<double>();

  const auto length_of_stay_mean =
      info_node["length_of_stay"]["mean"].as<double>();
...
void relative_bitting_info::set_value(const YAML::Node &node) {
  auto info_node = node[name_];
  value_.max_relative_biting_value =
      info_node["max_relative_biting_value"].as<double>();

  value_.number_of_biting_levels =
      info_node["number_of_biting_levels"].as<int>();

  value_.scale = info_node["biting_level_distribution"]["Exponential"]["scale"]
                     .as<double>();

  value_.mean =
      info_node["biting_level_distribution"]["Gamma"]["mean"].as<double>();
  value_.sd =
      info_node["biting_level_distribution"]["Gamma"]["sd"].as<double>();
  ...
  const auto max =
      value_.max_relative_biting_value - 1;  // maxRelativeBiting -1
  ...
  assert((unsigned)value_.number_of_biting_levels
...
void initial_parasite_info::set_value(const YAML::Node &node) {
  ...
  for (std::size_t index = 0; index < info_node.size(); index++) {
    ...
    const auto location_to =
        location == -1
            ...
            : std::min<int>(location + 1, config_->number_of_locations());

    // apply for all location, note that the compiler has a hard time guessing
    // the right type for auto here
    for (unsigned long loc = location_from; loc < location_to; ++loc) {
      for (std::size_t j = 0; j < info_node[index]["parasite_info"].size();
           j++) {
        auto parasite_type_id =
            info_node[index]["parasite_info"][j]["parasite_type_id"].as<int>();
        auto prevalence =
            info_node[index]["parasite_info"][j]["prevalence"].as<double>();
...
void bitting_level_generator::set_value(const YAML::Node &node) {
  value_.level_density =
      config_->relative_bitting_info().v_biting_level_density;
}
...
void start_of_comparison_period::set_value(const YAML::Node &node) {
  ...
  value_ =
      (date::sys_days{ymd} - date::sys_days(config_->starting_date())).count();
}

void prob_individual_present_at_mda_distribution::set_value(
    const YAML::Node &node) {
  value_.clear();
  for (std::size_t i = 0;
       i < config_->mean_prob_individual_present_at_mda().size(); i++) {
