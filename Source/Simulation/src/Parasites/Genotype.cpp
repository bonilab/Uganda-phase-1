...
Genotype::Genotype(const int &id, const GenotypeInfo &genotype_info,
  ...
  for (std::size_t i = 0; i < genotype_info.loci_vector.size(); i++) {
    daily_fitness_multiple_infection_ *= 1
                                         - genotype_info.loci_vector[i]
                                               .alleles[gene_expression_[i]]
                                               .daily_cost_of_resistance;
  }

  // number_of_resistance_position (level)
  number_of_resistance_position_ = 0;
  for (std::size_t i = 0; i < genotype_info.loci_vector.size(); i++) {
    number_of_resistance_position_ += genotype_info.loci_vector[i]
                                          .alleles[gene_expression_[i]]
                                          .mutation_level;
  }
}
...
bool Genotype::resist_to(DrugType* dt) {
  for (std::size_t i = 0; i < dt->affecting_loci().size(); i++) {
    for (std::size_t j = 0; j < dt->selecting_alleles()[i].size(); j++) {
      if (gene_expression_[dt->affecting_loci()[i]]
          == dt->selecting_alleles()[i][j]) {
        return true;
      }
    }
  }
...
bool Genotype::resist_to(Therapy* therapy) {
  ...
  if (sc_therapy != nullptr) {
    for (auto drug_id : sc_therapy->drug_ids) {
      if (resist_to(Model::CONFIG->drug_db()->at(drug_id))) { return true; }
    }
  }
  return false;
}
...
Genotype* Genotype::combine_mutation_to(const int &locus, const int &value) {
  if (gene_expression_[locus] == value) { return this; }
...
int Genotype::select_mutation_allele(const int &mutation_locus) {
  ...
  const int pos = static_cast<const int>(
      Model::RANDOM->random_uniform_int(0, Model::CONFIG->genotype_info()
                                               .loci_vector[mutation_locus]
                                               .alleles[current_allele_value]
                                               .mutation_values.size()));
  return Model::CONFIG->genotype_info()
      .loci_vector[mutation_locus]
      .alleles[current_allele_value]
      .mutation_values[pos];
}
...
std::string Genotype::to_string() { return to_string(Model::CONFIG); }
