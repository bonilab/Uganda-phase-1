...
class Person : public PersonIndexAllHandler,
               ...
               public PersonIndexByLocationBitingLevelHandler,
...
public:
  ...
  // if birthday is -100 which is that person was born 100 day before the
  // simulation start
  PROPERTY_REF(int, birthday)

  POINTER_PROPERTY(SingleHostClonalParasitePopulations,
...
public:
  ...
  void NotifyChange(const Property &property, const void* oldValue,
                    const void* newValue);

  virtual void increase_age_by_1_year();

  ClonalParasitePopulation* add_new_parasite_to_blood(
      Genotype* parasite_type) const;

  // Notify the individual of the change to the force of infection (FOI) for one
  // of their parasite clones.
  //
  // Sign - positive to increment the global FOI, negative to decrement the
  // global FOI blood_parasite_log_relative_density - The parasite density for a
  // single parasite clone in the individual.
  // log_total_relative_parasite_density - The total parasite density for all
  // clones present in the individual
  ...
  // Calculate the infectivity of an arbitrary parasite population based upon
  // the parasite density. This is based upon the calculations by Ross et al.
  // (2006)
  ...
  void schedule_progress_to_clinical_event_by(
      ClonalParasitePopulation* blood_parasite);

  void schedule_test_treatment_failure_event(
      ClonalParasitePopulation* blood_parasite, const int &testing_day,
      const int &t_id = 0);

  void schedule_update_by_drug_event(
      ClonalParasitePopulation* clinical_caused_parasite);
  ...
  void schedule_mature_gametocyte_event(
      ClonalParasitePopulation* clinical_caused_parasite);
